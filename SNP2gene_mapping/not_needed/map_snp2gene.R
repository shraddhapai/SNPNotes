...
tryCatch({
	...
	cat("Annovar")
	av <- read.delim(annovar,sep="\t",h=T,as.is=T)
	n <- nrow(av)
	av <- av[which(!av$avsnp147 %in% c(".")),]
	cat(sprintf("\thave snp id, %s of %s entries\n",
		pn(nrow(av)),pn(n)))
	idx1 <- which(av$ExonicFunc.refGene %in% 
		c("nonsynonymous SNV", "stopgain","stoploss")) 
	idx2 <- which(av$Func.refGene %in% 
		c("splicing","ncRNA_exonic","ncRNA_splicing"))
	idx <- union(idx1,idx2)
	cat(sprintf("\t%i ns/stop gain or loss ; %i affect splicing or ncRNA exonic\n\t%i total\n",
		length(idx1),length(idx2),length(idx)))
	av <- av[idx,]
	outFile <- sprintf("%s/annovar_%s.out",inDir,dt)
	write.table(av,file=outFile,sep="\t",col=T,row=F,quote=F)
	out[["annovar"]] <- av[,c("avsnp147","Gene.refGene")]
	colnames(out[["annovar"]]) <- c("snp","gene")
	out[["annovar"]] <- subset(out[["annovar"]], gene %in% brain)
